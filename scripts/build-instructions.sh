...
SRC="$PWD/docs/instructions/AgentsUnite-Instructions.html"
OUT="$PWD/docs/instructions/AgentsUnite-Instructions.pdf"
[ -x "$CHROME" ] || { echo "Chrome not found at $CHROME" >&2; exit 1; }
rm -f "$OUT"
# Separate throwaway profile so a running Chrome does not block headless mode.
# Headless Chrome sometimes lingers after writing the PDF, so run it in the
# background and stop it once the file is complete.
